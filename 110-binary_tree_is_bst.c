...
int binary_tree_is_bst(const binary_tree_t *tree)
{
	if (!tree)
		return (0);
	return (bt_ibst_helper(tree, INT_MIN, INT_MAX));
}

/**
 * bt_ibst_helper - checks if a binary tree is a valid Binary Search Tree
 * @tree: a pointer to the root node of the tree to check
...
int bt_ibst_helper(const binary_tree_t *tree, int min, int max)
{
	...
	return (bt_ibst_helper(tree->left, min, tree->n - 1) &&
		bt_ibst_helper(tree->right, tree->n + 1, max));
